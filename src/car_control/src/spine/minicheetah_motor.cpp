...
void MiniCheetahMotor::stop() {
  if (id_ != 0 && sc_ != nullptr) {
    ...
    frame1.can_id = id_;
    frame1.can_dlc = 8;
    uint16_t currentInt = tau2uint(tauDes_);
    uint16_t posInt = q2uint(0);
    uint16_t velInt = qd2Uint(0);
    uint16_t kpInt = kp2uint(kP_);
    uint16_t kdInt = kd2uint(kD_);
    frame1.data[0] = posInt >> 8;
    frame1.data[1] = posInt & 0xFF;
    frame1.data[2] = velInt >> 4;
    frame1.data[3] = ((velInt & 0xF) << 4) | ((kpInt) >> 8);
    frame1.data[4] = kpInt & 0xFF;
    frame1.data[5] = kdInt >> 4;
    frame1.data[6] = ((kdInt & 0xF) << 4) | (currentInt >> 8);
    frame1.data[7] = currentInt & 0xFF;
    *sc_ << frame1;
